...
class CityItemPipeline(object):
    ...
    def process_item(self, item, spider):
        if isinstance(item, CityItem):
            ...
            lis = (item['cityProvince'], item['cityName'], item['totalHotels'], item['totalComments'])
            self.cur.execute(sql, lis)
            self.client.commit()
            return item
        if isinstance(item, RestaurantItem):
            sql = 'insert into restaurant(name,rank,commands,score,address,phone,pics) values(%s,%s,%s,%s,%s,%s,%s)'
            lis = (item['name'], item['ranking'], item['commands'], item['score'], item['address'], item['phoneNumber'], item['pics'])
            self.cur.execute(sql, lis)
            self.client.commit()
            return item
